	...
	loadImageSource( sourceIndex, loader ) {
		...
		if ( sourceDef.bufferView !== undefined ) {
			...
			sourceURI = parser.getDependency( 'bufferView', sourceDef.bufferView ).then( function ( bufferView ) {
				...
				// Use a data: URL instead of a blob: URL. Under the file://
				// protocol, Chromium/WebKit blocks fetch() on blob: URLs
				// ("Fetch API cannot load blob:file:/// due to access
				// control checks"), which breaks embedded glTF textures.
				// data: URLs aren't fetched the same way and work fine.
				isObjectURL = false;
				let binary = '';
				...
				sourceURI = 'data:' + sourceDef.mimeType + ';base64,' + btoa( binary );
				return sourceURI;
